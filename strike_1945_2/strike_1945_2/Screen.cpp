...
void Cscreen::LoadBmpImg(char * filename, unsigned int MyImage)
{
	FILE* file;
	int padding = 0;
	unsigned char *temp;
	...
	{ // file opened
		...
		if (bmpImage[MyImage].ih.biSizeImage == 0)    // 픽셀 데이터 크기가 0이라면-------------------------------------------------------------------------------왜 0이면??
		{
			bmpImage[MyImage].ih.biSizeImage = (bmpImage[MyImage].ih.biWidth * _BI_BITFIELDS + padding) * bmpImage[MyImage].ih.biHeight;
			// 이미지의 가로 크기 * 픽셀 크기에 남는 공간을 곱해주면 가로 한 줄 크기가 나옴
			// 여기에 이미지의 세로 크기를 곱해주면 픽셀 데이터의 크기를 구할 수 있음
		}


		bmpImage[MyImage].pixels = (_RGBTRIPLE*)malloc(bmpImage[MyImage].ih.biHeight*bmpImage[MyImage].ih.biWidth * sizeof(_RGBTRIPLE));
		temp = (unsigned char*)malloc(bmpImage[MyImage].ih.biHeight*bmpImage[MyImage].ih.biWidth * sizeof(_RGBTRIPLE));    // 픽셀 데이터의 크기만큼 동적 할당

		fseek(file, bmpImage[MyImage].fh.bfOffBits, SEEK_SET); // 파일 포인터를 픽셀 데이터의 시작 위치로 이동
		if (fread(temp, sizeof(_RGBTRIPLE), bmpImage[MyImage].ih.biHeight*bmpImage[MyImage].ih.biWidth, file) < 1) // 파일에서 픽셀 데이터 크기만큼 읽기 실패하면 닫음.
		{
		...
		for (int y = bmpImage[MyImage].ih.biHeight - 1; y >= 0; y--) // 픽셀 데이터는 아래 위가 뒤집혀서 저장되므로 아래쪽부터 반복, 세로 크기만큼 반복
		{
