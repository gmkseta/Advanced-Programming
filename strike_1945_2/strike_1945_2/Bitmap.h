...
public:
	...
	void readBmpImage(const char* filename) {
		...
		if (file != NULL) { // file opened
			...
			padding = (_PIXEL_ALIGN - ((this->ih.biWidth * _BI_BITFIELDS) % _PIXEL_ALIGN)) % _PIXEL_ALIGN;

			if (this->ih.biSizeImage == 0)    // 픽셀 데이터 크기가 0이라면-------------------------------------------------------------------------------왜 0이면??
			{
				this->ih.biSizeImage = (this->ih.biWidth * _BI_BITFIELDS + padding) * this->ih.biHeight;
			...
			for (int y = this->ih.biHeight - 1; y >= 0; y--) // 픽셀 데이터는 아래 위가 뒤집혀서 저장되므로 아래쪽부터 반복, 세로 크기만큼 반복
			{
				for (int x = 0; x < this->ih.biWidth; x++) // 가로 크기만큼 반복
				{
					...
					int index = (x * _BI_BITFIELDS) + (y * (this->ih.biWidth* _BI_BITFIELDS)) + (padding * y);
